...
{ 
  ...
  *************************************************************************/ 
  ...
  if(argc != 3) {
    cout<<"para error:calibration_zed_mono [device id] [left or right]"<<endl;
    return -1;
  }
  ...
  while(n < image_count )
    {
      ...
      if(0 == camid)
	frame = frame(roi_rect0);
      else
	frame = frame(roi_rect1);
  ...
  *************************************************************************/   
  ...
  char strfile[32];
  sprintf(strfile,"zed_cam%d_calib.yml",camid);
  FileStorage fs1(strfile, FileStorage::WRITE);
  fs1 << "CM1" << intrinsic_matrix;
  fs1 << "D1" << distortion_coeffs;
  ...
  *************************************************************************/
  ...
  while(1) {    
    ...
    remap(img, imgU, mapx, mapy, INTER_LINEAR, BORDER_CONSTANT, Scalar());
